...
const ProductGallery: React.FC<{ product: Product, onClose: () => void }> = ({ product, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-rose-50/98 backdrop-blur-xl animate-fade-in no-print overflow-y-auto">
      <button 
        onClick={onClose} 
        className="absolute top-6 md:top-10 right-6 md:right-10 text-neutral-400 hover:text-rose-600 transition-colors z-[210] p-2 md:p-4 group"
      >
          ...
          <div className="w-full h-full relative flex items-center justify-center group">
            ...
            {images.length > 1 && (
              <div className="absolute inset-x-0 top-1/2 -translate-y-1/2 flex justify-between px-2 md:px-10 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity duration-500">
                <button onClick={() => setActiveIndex((activeIndex - 1 + images.length) % images.length)} className="p-4 md:p-6 bg-white/80 hover:bg-white text-rose-900 rounded-full shadow-lg backdrop-blur-sm transition-all"><svg className="w-5 h-5 md:w-6 md:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M15 19l-7-7 7-7" strokeWidth="1.5"/></svg></button>
                <button onClick={() => setActiveIndex((activeIndex + 1) % images.length)} className="p-4 md:p-6 bg-white/80 hover:bg-white text-rose-900 rounded-full shadow-lg backdrop-blur-sm transition-all"><svg className="w-5 h-5 md:w-6 md:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M9 5l7 7-7 7" strokeWidth="1.5"/></svg></button>
              </div>
            )}
          </div>
        </div>

        <div className="w-full lg:w-[450px] flex flex-col justify-center border-t lg:border-t-0 lg:border-l border-rose-100 pt-8 lg:pt-0 lg:pl-20 pb-8 lg:pb-0">
           <div className="mb-8 md:mb-10">
             <span className="text-rose-600 font-bold tracking-[0.4em] uppercase text-[9px] md:text-[10px] mb-3 md:mb-4 block">Atelier Showcase</span>
             ...
             <div className="flex items-center gap-4 md:gap-6 mb-8 md:mb-12">
               <span className="text-xl md:text-2xl font-serif text-rose-800">{product.price}</span>
               <span className="h-px w-8 md:w-12 bg-rose-200"></span>
           ...
           <button onClick={() => alert('Our atelier will contact you for measurement and shipping specifications.')} className="w-full py-4 md:py-6 bg-neutral-900 text-white rounded-none font-black text-[10px] md:text-[11px] uppercase tracking-widest hover:bg-rose-700 transition-all shadow-xl">Inquire / Custom Order</button>
        </div>
      </div>
    </div>
  );
...
const Boutique: React.FC = () => {
  ...
  return (
    <div className="animate-fade-in bg-rose-50/20 min-h-screen pb-20 md:pb-32 overflow-x-hidden">
      <section className="relative min-h-[60vh] md:min-h-screen flex items-center bg-rose-50/30 overflow-hidden pt-16 md:pt-20 pb-12 md:pb-0">
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-12 items-center gap-12 md:gap-20">
          <div className="lg:col-span-6 z-10">
            <EditableText id="boutique_luxury_badge" defaultText="The AK Luxury Atelier" className="text-rose-600 font-bold uppercase tracking-[0.5em] text-[8px] md:text-[10px] block mb-6 md:mb-10" />
            <h1 className="text-5xl sm:text-7xl md:text-[10rem] font-serif text-neutral-900 mb-8 md:mb-12 leading-[1.1] md:leading-[0.8] tracking-tighter">Sale <br/><span className="italic text-rose-400">Point</span></h1>
            <div className="max-w-md border-l-2 md:border-l border-rose-900 pl-4 md:pl-10 pt-2 md:pt-4">
      ...
      <section id="collection-grid" className="py-8 md:py-16 bg-white/90 sticky top-[60px] md:top-[72px] z-40 border-b border-rose-100 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-6 flex flex-wrap justify-center items-center gap-6 sm:gap-12 lg:gap-24">
          {[
            ...
            <button 
              ...
              className={`text-[8px] md:text-[10px] font-black uppercase tracking-[0.4em] transition-all relative py-2 ${filter === cat.value ? 'text-rose-600 after:content-[""] after:absolute after:bottom-0 after:left-0 after:w-full after:h-[2px] after:bg-rose-600' : 'text-neutral-400 hover:text-rose-900'}`}
            >
              {cat.label}
            </button>
          ))}
        ...
        <div className="max-w-7xl mx-auto px-6">
            ...
            {filteredProducts.map((product) => {
              return (
                <div key={product.id} className="group flex flex-col">
                  <div 
                    className="relative aspect-[3/4] overflow-hidden mb-6 md:mb-8 shadow-sm group-hover:shadow-2xl transition-all duration-700 cursor-zoom-in rounded-[2rem] bg-rose-50" 
                  ...
                  <div className="flex flex-col justify-between items-start gap-3 px-2">
                    <div className="w-full">
                      <div className="flex justify-between items-center mb-1">
                        ...
                        <p className="text-rose-700 font-serif text-base md:text-lg block font-bold">{product.price}</p>
                      </div>
          ...
          {filteredProducts.length === 0 && (
            <div className="py-20 text-center">
              <p className="text-neutral-400 font-serif italic text-xl">New collection launching soon...</p>
            </div>
          )}
        </div>
      </section>
