...
const InstitutionalClassroom: React.FC<{ user: UserProfile | Student }> = ({ user }) => {
  ...
  useEffect(() => {
    ...
    if (mainEl) mainEl.srcObject = swapped ? studentStream : instructorStream;
    if (guestEl) guestEl.srcObject = swapped ? instructorStream : studentStream;
      ...
      <div className="flex-grow space-y-8 md:space-y-10">
        ...
        {sessionMode === 'media' && mediaUrl && (
          <div className="w-full bg-slate-900 p-3 rounded-3xl border border-slate-800 shadow-lg animate-fade-in">
            <video src={mediaUrl} controls className="w-full h-12 bg-transparent overflow-hidden" style={{ filter: 'invert(1) hue-rotate(180deg) contrast(1.5)' }} />
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-3 md:gap-4 px-2">
            {participants.map(p => (
              <MicroParticipantCard key={p.id} participant={p} isAdmin={isAdmin} onSpotlight={handleSpotlight} onMute={handleToggleMute} onCam={handleToggleCam} />
            ))}
          </div>
        </div>
      </div>
...
const DatabaseSyncTool: React.FC = () => {
  const [status, setStatus] = useState<'idle' | 'loading' | 'success' | 'error'>('idle');
  const [message, setMessage] = useState('');

  const handleSeed = async () => {
    if (!confirm("This will synchronize the remote curriculum registry with local defaults. Continue?")) return;
    setStatus('loading');
    const result = await mockAuth.seedDatabase();
  ...
  return (
    <div className="p-10 animate-fade-in max-w-2xl">
      <h3 className="text-2xl font-serif text-slate-900 mb-6">Database Synchronization</h3>
      <div className="space-y-8">
        <div className={`p-8 rounded-[2rem] border ${isSupabaseConfigured ? 'bg-emerald-50 border-emerald-100' : 'bg-rose-50 border-rose-100'}`}>
           <div className="flex items-center gap-4 mb-4">
              <div className={`w-3 h-3 rounded-full animate-pulse ${isSupabaseConfigured ? 'bg-emerald-500' : 'bg-rose-500'}`}></div>
              <p className="text-[10px] font-black uppercase tracking-widest">{isSupabaseConfigured ? 'Supabase Connected' : 'Disconnected / Local Mode'}</p>
           </div>
           <p className="text-sm text-slate-500 font-light leading-relaxed">
             {isSupabaseConfigured 
               ? "The system is linked to your remote data cloud. You can push local curriculum updates to the live server." 
               : "No active cloud project detected. Application is running using hardcoded fallback records."}
           </p>
        </div>

        {isSupabaseConfigured && (
          <div className="space-y-6">
            <p className="text-xs text-slate-400 uppercase font-bold tracking-widest">Maintenance Protocols</p>
            <button 
              onClick={handleSeed}
              disabled={status === 'loading'}
              className="px-10 py-5 bg-teal-800 text-white rounded-full font-black text-[10px] uppercase tracking-widest shadow-xl hover:bg-teal-900 transition-all disabled:opacity-50"
            >
              Initialize Remote Curriculum
            </button>
            {status !== 'idle' && (
              <div className={`mt-6 p-6 rounded-2xl text-[10px] font-black uppercase tracking-widest text-center ${status === 'success' ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>
                {message}
              </div>
            )}
          </div>
...
const LMS: React.FC<{ externalUser: (UserProfile | Student) | null; onAuthUpdate: (u: (UserProfile | Student) | null) => void }> = ({ externalUser, onAuthUpdate }) => {
  ...
  useEffect(() => { 
    if (!externalUser) { terminateSession(); navigate('/#portal-hub'); }
  }, [externalUser, navigate, terminateSession]);
  if (!externalUser) return null;
  const handleSignOut = () => { terminateSession(); onAuthUpdate(null); };
  return (
...
const CourseManager: React.FC = () => {
  ...
  const [form, setForm] = useState<Omit<Course, 'id'>>({
    title: '', description: '', category: 'Vocational', duration: '6 Months', image: '', status: 'active', contents: []
  });
  ...
  const handleEditInit = (c: Course) => {
    setEditingId(c.id);
    setForm(c);
    setContentInput(c.contents?.join(', ') || '');
    setIsAdding(true);
  };

  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    const processedContents = contentInput.split(',').map(s => s.trim()).filter(s => s !== '');
    const courseToSave = { ...form, contents: processedContents };
    
    if (editingId) await mockAuth.updateCourse(editingId, courseToSave);
    else await mockAuth.createCourse(courseToSave);
  ...
  return (
    <div className="p-6 md:p-10 animate-fade-in">
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mb-8 md:mb-12">
        ...
        <button onClick={() => { setIsAdding(true); setEditingId(null); setForm({title: '', description: '', category: 'Vocational', duration: '6 Months', image: '', status: 'active', contents: []}); setContentInput(''); }} className="w-full sm:w-auto px-8 py-3 bg-teal-800 text-white rounded-full text-[9px] md:text-[10px] font-black uppercase tracking-widest shadow-xl">Define New Subject</button>
      </div>
        ...
        {courses.map(c => (
          <div key={c.id} className="bg-slate-50 p-5 md:p-6 rounded-[2rem] md:rounded-[2.5rem] border border-slate-100 group transition-all hover:bg-white hover:shadow-2xl">
            ...
            <div className="flex gap-4 border-t border-slate-200 pt-4 md:pt-6">
               <button onClick={() => handleEditInit(c)} className="text-[8px] md:text-[9px] font-black uppercase text-teal-700 hover:underline">Edit</button>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
          <form onSubmit={handleSave} className="bg-white p-8 md:p-12 rounded-[3rem] md:rounded-[4rem] w-full max-w-xl space-y-4 md:space-y-6 shadow-2xl overflow-y-auto max-h-[90vh]">
            ...
            <div className="space-y-3 md:space-y-4">
              ...
              <input placeholder="Duration (e.g. 6 Months)" className="w-full p-4 md:p-5 bg-slate-50 border-none rounded-2xl" value={form.duration} onChange={e => setForm({...form, duration: e.target.value})} />
              <div className="space-y-1">
                <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-4">Curriculum Modules (Comma Separated)</label>
                <textarea 
                  placeholder="Module 1, Module 2, Module 3..." 
                  className="w-full p-4 md:p-5 bg-slate-50 border-none rounded-2xl h-24 resize-none" 
                  value={contentInput} 
...
const RegistryPanel: React.FC = () => {
  ...
  const [activeSub, setActiveSub] = useState<'applications' | 'courses' | 'system'>('applications');
  ...
  return (
    <div className="p-6 md:p-10">
      <div className="flex gap-2 md:gap-4 mb-8 md:mb-12 bg-slate-50 p-1.5 md:p-2 rounded-[1.5rem] md:rounded-[2rem] w-fit border border-slate-100">
        ...
        <button onClick={() => setActiveSub('system')} className={`px-6 md:px-10 py-2.5 md:py-3 rounded-2xl text-[9px] md:text-[10px] font-black uppercase tracking-widest transition-all ${activeSub === 'system' ? 'bg-white text-teal-800 shadow-xl' : 'text-slate-400'}`}>System</button>
      </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-slate-50">
                {pending.map(s => (
                  <tr key={(s as any).id} className="hover:bg-slate-50/50 transition-colors">
      ...
      ) : activeSub === 'courses' ? <CourseManager /> : <DatabaseSyncTool />}
    </div>
  );
};
